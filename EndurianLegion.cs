...
{
    public class EndurianLegion
    {

        //List of PreHardmodeInvaders
        public static int[] Star = {
            ModLoader.GetMod("EnduriumMod").NPCType("JadeWalker"),
            ModLoader.GetMod("EnduriumMod").NPCType("ChargedGraniteCaster"),
            ModLoader.GetMod("EnduriumMod").NPCType("JadeWarrior"),
            ModLoader.GetMod("EnduriumMod").NPCType("JadeWorm_Head"),
            ModLoader.GetMod("EnduriumMod").NPCType("JadeWrath"),
            ModLoader.GetMod("EnduriumMod").NPCType("JadeWarrior")
        };
		...
		public static void StartDungeonInvasion()
		{
			...
			if (Main.invasionType == 0)
			{
				...
				if (num > 0)
				{
					//Invasion setup
					Main.invasionType = -1;
					EnduriumWorld.EndurianLegion = true;
		...
		public static void DungeonInvasionWarning()
		{
			...
			{
				text = "The Tribe is approaching";
			}
			if(Main.invasionSize <= 0)
			{
				text = "The Tribe has been defeated";
			}
			if (Main.netMode == 0)
			{
		...
		{
			if(EnduriumWorld.LegionRemainingPoints >= 0)
			{
				//End invasion if size less or equal to 0
				if(Main.invasionSize <= 0)
				{
					EnduriumWorld.LegionRemainingPoints = 0;
		...
		public static void CheckDungeonInvasionProgress()
		{
			...
			if(EnduriumWorld.LegionRemainingPoints >= 0 && (Main.invasionX == (double)Main.spawnTileX))
			{
