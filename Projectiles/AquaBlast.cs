...
namespace EnduriumMod.Projectiles
{
    public class AquaBlast : ModProjectile
    {
        ...
        {
            DisplayName.SetDefault("Aquatic Spirits");
        }
        ...
        public override void Kill(int timeLeft)
        {
            for (int k = 0; k < 5; k++)
            {
                Dust.NewDust(projectile.position + projectile.velocity, projectile.width, projectile.height, 156, projectile.oldVelocity.X * 0.5f, projectile.oldVelocity.Y * 0.3f);
            }
        }
            ...
            if (Main.rand.Next(8) == 0)
            {
                if (projectile.owner == Main.myPlayer)
                {
                    ...
                    for (int num627 = 0; num627 < num626; num627 = num3 + 5)
                    {
                        ...
                        num628 *= 20f;
                        num629 *= 20f;
                        Projectile.NewProjectile(projectile.position.X, projectile.position.Y, num628, num629, mod.ProjectileType("AquaBlast"), (int)((double)projectile.damage * 0.75), (float)((int)((double)projectile.knockBack * 0.35)), Main.myPlayer, 0f, 0f);
                        num3 = num627;
                    }
                }
            }
