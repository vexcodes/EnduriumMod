...
{
    public class ErodedJavelin : ModProjectile
    {
        ...
        public override void Kill(int timeLeft)
        {
            for (int i = 4; i < 31; i++)
            {
                ...
                int newDust = Dust.NewDust(new Vector2(projectile.oldPosition.X - x, projectile.oldPosition.Y - y), 8, 8, 21, projectile.oldVelocity.X, projectile.oldVelocity.Y, 58, default(Color), 2.2f);
        ...
        {                                                           // sound that the projectile make when hiting the terrain
            {
                projectile.Kill();
                int a = Dust.NewDust(projectile.position + projectile.velocity, projectile.width, projectile.height, 21, projectile.velocity.X * 2.5f, projectile.velocity.Y * 2.5f);
