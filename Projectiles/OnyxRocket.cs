...
{
    public class OnyxRocket : ModProjectile
    {
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Onyx Rocket");
        }
        ...
        public override void AI()
        {
            ...
            if (projectile.alpha <= 0)
            {
                ...
                {
                    int num109 = Dust.NewDust(projectile.position, projectile.width, projectile.height, 240, 0f, 0f, 0, default(Color), 0.8f);
            ...
            if (projectile.alpha > 0)
            {
                ...
                if (projectile.alpha < 0)
                {
                    ...
                    while ((float)num111 < num110)
                    {
                        ...
                        int num112 = Dust.NewDust(projectile.Center, 0, 0, 62, 0f, 0f, 0, default(Color), 1f);
        ...
        public override void Kill(int timeLeft)
        {
            for (int k = 0; k < 15; k++)
            {
                int num137 = Dust.NewDust(projectile.position + projectile.velocity, projectile.width, projectile.height, 70, projectile.oldVelocity.X * 0.05f, projectile.oldVelocity.Y * 0.05f);
                Main.dust[num137].noGravity = true;
            }
            for (int k = 0; k < 15; k++)
            {
                int num138 = Dust.NewDust(projectile.position + projectile.velocity, projectile.width, projectile.height, 54, projectile.oldVelocity.X * 0.05f, projectile.oldVelocity.Y * 0.05f);
                Main.dust[num138].noGravity = true;
            }
            for (int k = 0; k < 15; k++)
            {
                int num139 = Dust.NewDust(projectile.position + projectile.velocity, projectile.width, projectile.height, 52, projectile.oldVelocity.X * 0.05f, projectile.oldVelocity.Y * 0.05f);
