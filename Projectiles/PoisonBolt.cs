...
{
    public class PoisonBolt : ModProjectile
    {
        ...
        {
            DisplayName.SetDefault("Poison Bolt");
        }
        ...
        public override bool PreAI()
        {
            ...
            for (int num93 = 0; num93 < 5; num93 = num3 + 1)
            {
                ...
                int num96 = 4;
                int num97 = Dust.NewDust(new Vector2(projectile.position.X + (float)num96, projectile.position.Y + (float)num96), projectile.width - num96 * 2, projectile.height - num96 * 2, 116, 0f, 0f, 100, default(Color), 0.6f);
        ...
        public override void Kill(int timeLeft)
        {
            for (int k = 0; k < 5; k++)
            {
                Dust.NewDust(projectile.position + projectile.velocity, projectile.width, projectile.height, 116, projectile.oldVelocity.X * 0.5f, projectile.oldVelocity.Y * 0.3f);
            }
        }
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            ...
            for (int j = 0; j < num11; j++)
            {
                ...
                Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, vector.X, vector.Y, mod.ProjectileType("PoisonBolt2"), num12, projectile.knockBack * 0.2f, projectile.owner, 0f, 0f);
            }
        }
    }
}