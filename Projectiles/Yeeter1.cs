...
{
    public class Yeeter1 : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                int a = Dust.NewDust(projectile.position + projectile.velocity, projectile.width, projectile.height, 75, projectile.velocity.X * 0.5f, projectile.velocity.Y * 0.5f);
            ...
            if (projectile.ai[0] != 0)
            {
                ...
                {
                    int num3 = 2;
                    for (int num731 = 0; num731 < 2; num731 += 1)
                    {
                        Projectile.NewProjectile((projectile.Center.X + (projectile.velocity.X * 1.4f)) + Main.rand.Next(-25, 26), (projectile.Center.Y + (projectile.velocity.Y * 1.4f)) + Main.rand.Next(-25, 26), 0f, 0f, mod.ProjectileType("Yeeter1Explosion"), 60, 0f, Main.myPlayer, 0f, 0f);
                    }
                    projectile.Kill();
                }
            }
        ...
        public override void Kill(int timeLeft)
        {
            ...
            if (modPlayer.SpearBoom == 0)
            {
                ...
                for (int num731 = 0; num731 < 22; num731 = num3 + 1)
                {
                    ...
                    int num732 = Dust.NewDust(new Vector2(projectile.oldPosition.X - x, projectile.oldPosition.Y - y), projectile.width, projectile.height, 75, 0f, 0f, 100, default(Color), 1.5f);
                    Main.dust[num732].noGravity = true;
                    Dust dust = Main.dust[num732];
                    dust.velocity.X *= 0.8f;
                    dust.velocity.Y *= 1.2f;
                    num732 = Dust.NewDust(new Vector2(projectile.oldPosition.X - x, projectile.oldPosition.Y - y), projectile.width, projectile.height, 75, 0f, 0f, 100, default(Color), 0.8f);
