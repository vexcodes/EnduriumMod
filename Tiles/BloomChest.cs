...
namespace EnduriumMod.Tiles
{
	class BloomChest : ModTile
	{
        ...
        {
            Terraria.Item.NewItem(i * 16, j * 16, 32, 32, mod.ItemType("BloomChest"));
        ...
        public override void RightClick(int i, int j)
        {
            ...
            {
                if (player.HasItem(mod.ItemType("BloomKey")) || tile.frameX == 36)//If the player has a key or if the chest was unlocked in legacy (remove the second check to discontinue legacy format support)
                {
        ...
        public override void MouseOver(int i, int j)
        {
            ...
            {
                player.showItemIconText = Main.chest[chest].name.Length > 0 ? Main.chest[chest].name : "Bloom Chest";
                if (player.showItemIconText == "Bloom Chest")
                {
                    if (tile.frameX >= 36)
                    {
                        player.showItemIcon2 = mod.ItemType("BloomKey");
                        player.showItemIconText = "";
                    }
                    else
                    {
                        player.showItemIcon2 = mod.ItemType("BloomChest");
                    }
                }
            }
            player.noThrow = 2;
